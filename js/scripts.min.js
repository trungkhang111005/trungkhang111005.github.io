...
!function(n){
    ...
    n("header a").click(function(e){
        if(!n(this).hasClass("no-scroll")){
            e.preventDefault();
            var t=n(this).attr("href"),i=n(t).offset().top;
            n("html, body").animate({scrollTop:i+"px"},+Math.abs(window.pageYOffset-n(t).offset().top)),
    ...
    n("#experience-timeline").each(function(){
        $this=n(this),$userContent=$this.children("div"),
    ...
    n("#mobile-menu-open").click(function(){
        n("#menu").addClass("open")
    }),
    
    n("#mobile-menu-close").click(function(){
        n("#menu").removeClass("open")
    }),
    
    n(document).click(function(e) {
        if (!n("#menu").is(e.target) && !n("#mobile-menu-open").is(e.target) && n("#menu").has(e.target).length === 0) {
            n("#menu").removeClass("open");
        }
    }),
